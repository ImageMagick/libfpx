...
FPXStatus ViewImage::Read4x4Points (float x0, float y0, float x1, float y1, Pixel* bufPix)
{
  ...
  if ((x0 < 0.0) || (y0 < 0.0) || (x1 > finalWidth) || (y1 > finalHeight)) {
    ...
    float x, y;
    int32_t* pt = (int32_t*)(bufPix);
    int32_t* bk = (int32_t*)(backGround);
    y = y0; 
    for (long i = 0; i < 4; ++i) {
      x = x0;
      for (long j = 0; j < 4; ++j) {
...
void ViewWindow::WindowToWorld (int i, int j, float* x, float* y)
{
...
void ViewWindow::WorldToWindow (float x, float y, int* i, int* j)
{
